...
/* Basically identical to the Aaron/Linh problem but with a 2D array in column-major order */
...
/*
 * Linh wants the average sensor value for tree number 7.
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < NTREES; i++) {
		ssize_t bytes = read(fd, ecodata[i], M_PER_DAY * DAYS * sizeof(double));
		if (bytes  != M_PER_DAY * DAYS * sizeof(double)) {
			perror("Read failed");
			exit(1);
		}
	}	
	...
	/*
	   So, now we have a large chunk-o-data. Let's simulate Linh and
	   Aaron's analyses. Note: Linh and Aaron are both real live ecologists
	   with whom I work.
	*/
